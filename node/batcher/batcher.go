...
func (b *Batcher) MonitoringServiceAddress() string {
	return operations.PrometheusMetricsServiceURL(b.opsSystem, b.logger)
}
...
func (b *Batcher) CreateBAF(seq types.BatchSequence, primary types.PartyID, shard types.ShardID, digest []byte, txCount uint64, primarySignature []byte) types.BatchAttestationFragment {
	baf, err := CreateBAF(b.signer, b.config.PartyId, shard, digest, primary, seq, b.ConfigSequence(), txCount, primarySignature)
	if err != nil {
...
func CreateBAF(signer Signer, id types.PartyID, shard types.ShardID, digest []byte, primary types.PartyID, seq types.BatchSequence, configSeq types.ConfigSequence, txCount uint64, primarySignature []byte) (types.BatchAttestationFragment, error) {
	baf := types.NewSimpleBatchAttestationFragment(shard, primary, seq, digest, id, configSeq, txCount, primarySignature)
